...
# ==========================================
# 1. THE BACKEND LOGIC (DevRace Class)
# ==========================================
class DevRace:
    def __init__(self):
        self.db = sqlite3.connect("server/drdb.db")
    ...
    def register_user(self, username, password, first_name, last_name, age, country):
        if self.retrieve_user(username): return False
        try:
            hashed_pw = bcrypt.hashpw(password[:72].encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
            self.db.execute("""
    ...
    def get_rank_info(self, xp):
        if xp >= 38000: return "legend", ((xp - 38000) // 10000) + 1
        current_tier, current_div = "novice", 3
        for threshold, tier, div in self.THRESHOLDS:
            if xp >= threshold: current_tier, current_div = tier, div
            else: break
    ...
    def update_xp(self, username, gained_xp):
        ...
        new_xp, new_legacy = user['current_xp'] + gained_xp, user['legacy_xp'] + gained_xp
        new_tier, new_div = self.get_rank_info(new_xp)
        self.db.execute("UPDATE users SET current_xp=?, legacy_xp=?, tier=?, division=? WHERE username=?;", 
                        (new_xp, new_legacy, new_tier, new_div, username.lower()))
        self.db.commit()

    def generate_question(self, username):
        rank = self.retrieve_user(username)["tier"]
        topics = self.get_user_topics(username)
        
        # CHANGED: Removed the 'NOT IN (SELECT question_id...)' constraint to allow infinite looping
        if not topics:
        ...
        return self.db.execute(f"""
            SELECT * FROM questions WHERE category IN ({placeholders}) AND difficulty_tier = ? ORDER BY RANDOM() LIMIT 1
        """, (*topics, rank)).fetchone()
    ...
    def edit_user(self, username, first_name, last_name, age, country):
        try:
            self.db.execute("UPDATE users SET first_name=?, last_name=?, age=?, country=? WHERE username=?;", 
    ...
    def get_user_topics(self, username):
        return [row[0] for row in self.db.execute("SELECT t.topic_name FROM topics t JOIN users_topics ut ON t.topic_id = ut.topic_id WHERE ut.username = ?", (username.lower(),)).fetchall()]
    ...
    def update_user_topics(self, username, selected_topics):
        ...
        for t in selected_topics:
            t_id = self.db.execute("SELECT topic_id FROM topics WHERE topic_name=?", (t,)).fetchone()['topic_id']
